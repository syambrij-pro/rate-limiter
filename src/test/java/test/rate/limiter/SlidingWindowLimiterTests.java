...
import test.rate.limiter.service.SlidingWindowLimiter;

@ExtendWith(SpringExtension.class)
@TestInstance(Lifecycle.PER_CLASS)
public class SlidingWindowLimiterTests extends AbstractRateLimiterTests {
	
	@Test
	public void testSlidingWimdow() {
		LimitConfiguration limitConfig = createLimitConfig("SLIDING_WINDOW_USER", "test1", RateLimitPolicy.SLIDING_WINDOW);
		Limiter bucketLimiter = new SlidingWindowLimiter(limitConfig);
		ThrottlingReq throttlingReq = createThrottlingReq("SLIDING_WINDOW_USER", "test1");
	...
	public void testSlidingWimdowWithSleep() throws InterruptedException {
		LimitConfiguration limitConfig = createLimitConfig("SLIDING_WINDOW_USER_SLEEP", "test1", RateLimitPolicy.SLIDING_WINDOW);
		ThrottlingReq throttlingReq = createThrottlingReq("SLIDING_WINDOW_USER_SLEEP", "test1");
		Limiter bucketLimiter = new SlidingWindowLimiter(limitConfig);
		int trueCount = 0;
		for (int i = 0; i < 150; i++) {
			if (bucketLimiter.tryAcquire(throttlingReq)) {
	...
	@Test
	public void testSlidingWimdowWithInterval() throws InterruptedException {
